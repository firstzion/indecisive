...
extension Skin {
    ...
    /// labels. Gumball and the 8-Ball use their display font even here; the
    /// Wheel's Titan One reads too heavy at this density, so it drops to
    ...
    func compactTitleFont(_ size: CGFloat, weight: SkinFontWeight = .bold) -> Font {
        switch id {
        case .gumball, .eightBall: return type.display(size, weight: weight, relativeTo: .body)
